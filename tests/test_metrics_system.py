...
class TestOtlHandler(unittest.TestCase):
    ...
    def test_import_otl(self, mock_stdout, mock_input):
        ...
        species_col = {'species': ['species A', 'species B', 'species C', 'species D', 'species E', 'species Z']}
...
class TestReferenceDatabaseQuality(unittest.TestCase):
    def setUp(self):
        ...
        self.otl = str(self.test_directory / "test_otl.tsv")
        self.ref_bd_cls = ReferenceDatabaseQuality(all_inserts_path=self.inserts_file, otl=self.otl)
        self.handler = OtlHandler(self.otl)
    ...
    def test_calculate_number_of_barcodes_per_taxon_input(
        ...
        with patch("src.marker_scoring.metrics_system.OtlHandler") as mock_otl_handler:
            ...
            with patch("sys.exit") as mock_exit:
                ...
                try:
                    barcodes_per_entry = no_file_class.calculate_number_of_barcodes_per_fasta_entry()
                    no_file_class.calculate_number_of_barcodes_per_otl_taxonomy(barcodes_per_entry, no_file_class.taxa_hierarchy)
                except SystemExit:
                    pass
    ...
    def test_calculate_number_of_barcodes_per_taxon(self):
        barcodes_per_entry = self.ref_bd_cls.calculate_number_of_barcodes_per_fasta_entry()
        taxa_hierarchy =  self.ref_bd_cls.taxa_hierarchy
        barcodes = self.ref_bd_cls.calculate_number_of_barcodes_per_otl_taxonomy(barcodes_per_entry, taxa_hierarchy)

        expected_barcodes = {'test_amplicon_reffb': {
        'familyA': {'genera':
                     {'genusA': {'species':
                                 {'species A': {'count': 8}}, 'count': 8}}, 'count': 8},
        'familyB': {'genera':
                    {'genusB': {'species':
                                {'species B': {'count': 2}}, 'count': 2}}, 'count': 2},
        'familyC': {'genera':
                    {'genusC': {'species':
                                {'species C': {'count': 1}}, 'count': 1}}, 'count': 1},
        'familyD': {'genera':
                    {'genusD': {'species':
                                {'species D': {'count': 2}}, 'count': 2}}, 'count': 2},
        'familyE': {'genera':
                    {'genusE': {'species':
                                {'species E': {'count': 0}}, 'count': 0}}, 'count': 0},
        'familyZ': {'genera':
                    {'genusZ': {'species':
                                {'species Z': {'count': 0}}, 'count': 0}}, 'count': 0}
        }}
...
class TestBinding(unittest.TestCase):
    ...
    def test_process_analysis_per_taxon(self):
        ...
        expected = pd.DataFrame({
            "family": ["familyA", "familyB", "familyC", "familyD", "familyE", "familyZ"],
            "genus": ["genusA", "genusB", "genusC", "genusD", "genusE", "genusZ"],
            "species": ["species A", "species B", "species C", "species D", "species E", "species Z"],
