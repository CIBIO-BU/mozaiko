...
class TestCrabsScriptGenerator(unittest.TestCase):
    ...
    def test_check_if_crabs_installed_not_installed(self, mock_run):
        ...
        mock_run.assert_called_once_with(["crabs", "-h"], check=True, stdout=-3, stderr=-3)

    @patch("builtins.open", new_callable=mock_open, read_data='{"input": "input.fasta"}')
    @patch('json.load')
    ...
    @patch('os.chdir')
    @patch('subprocess.run')
    def test_download_taxonomy_files_not_exists(self, mock_run, mock_chdir, mock_mkdir, mock_path_exists):
        self.generator._download_taxonomy_files()
        mock_mkdir.assert_called_with("taxonomy_files")
        mock_chdir.assert_called_with("taxonomy_files")
        mock_run.assert_called_with("crabs db_download --source taxonomy", shell=True, check=True)

    @patch('subprocess.run')
    @patch('src.reference_database.db_curation.CrabsScriptGenerator._download_taxonomy_files')
    @patch('src.reference_database.db_curation.CrabsScriptGenerator._load_parameters')
    def test_run_assign_tax_command(self, mock_load_params, mock_download_files, mock_run):
