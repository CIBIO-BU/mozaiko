...
def tsv2fasta(tsv_file, fasta_file):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fasta_file', '-f',
                        help='The ouput FASTA file.')
    parser.add_argument('--output_fasta_file', '-o',
                       help='The output FASTA file without alignment dashes.')
    parser.add_argument('--task', '-a', choices=['tsv2fasta', 'remove_alignment_dashes'], 
                        help='The task to perform: tsv2fasta or remove_alignment_dashes')
    args = parser.parse_args()

    if args.task == 'tsv2fasta':
        tsv2fasta(args.tsv_file, args.fasta_file)
